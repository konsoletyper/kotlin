...
class InvokeIntrinsic : Intrinsic {
    override fun isApplicable(descriptor: FunctionDescriptor, asStatement: Boolean): Boolean {
        if (asStatement) return false

        return descriptor.name.asString() == "invoke" &&
               descriptor.containingDeclaration.fqNameSafe.asString().startsWith("kotlin.Function")
    }

    override fun apply(invocation: JsirExpression.Invocation): JsirExpression {
        return JsirExpression.Application(invocation.receiver!!, *invocation.arguments.toTypedArray())
    }
