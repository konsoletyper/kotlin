...
class JsirMutableContext(val bindingTrace: BindingTrace, val generator: (KtExpression) -> JsirExpression) : JsirContext {
    private var resultingStatements = mutableListOf<JsirStatement>()
    private var currentFunction: FunctionDescriptor? = null
    private var currentDeclaration: DeclarationDescriptor? = null
    ...
    override val declaration: DeclarationDescriptor
        get() = currentDeclaration ?: throw IllegalStateException("Context has not entered any declaration yet")

    override val function: FunctionDescriptor
        get() = currentFunction ?: throw IllegalStateException("Context has not entered any function declaration yet")

    override val classDescriptor: ClassDescriptor?
        get() = currentClass

    override val pool = JsirPool()
    ...
    override fun getVariable(descriptor: VariableDescriptor): LocalVariableAccessor {
    ...
    override fun nestedFunction(function: FunctionDescriptor, action: () -> Unit) {
    ...
    override fun generate(expression: KtExpression) = generator(expression)
